...
class _SalesPageState extends State<SalesPage> {
  ...
  void populateMonthNames() {
    //Setting 5 Months for Bar charts and their name
    final currentMonth = DateTime.now().month;
    for (int i = -2; i <= 2; i++) {
      final month = (currentMonth + i) % 12 == 0 ? 12 : (currentMonth + i) % 12;
      monthNames.add(DateFormat('MMM').format(DateTime(DateTime.now().year, month)));
    }
  }
  ...
  void calculateMonthlyData() {
    //Running loop and calculating Monthly Sale,Purchase and Profit
    final currentMonth = DateTime.now().month;
    final List<int> months = [];
    for (int i = -2; i <= 2; i++) {
      months.add((currentMonth + i) % 12 == 0 ? 12 : (currentMonth + i) % 12);
    }

    monthlyData = months.map((month) {
      final monthlyTransactions = transactions.where((transaction) {
        final transactionMonth =
            DateFormat('dd MMMM yyyy').parse(transaction.transactionDate!).month;
                                      ...
                                      children: [
                                        InkWell(
                                          ...
                                          child: Container(
                                            ...
                                            child: isInvoiceOpening ? CircularProgressIndicator(color: primaryColor,):Row(
